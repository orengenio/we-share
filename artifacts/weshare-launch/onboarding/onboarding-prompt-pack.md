...
**Before activating:**
1. Attach ONE credential — **the location-level PIT with all scopes** (verified live 2026-07-26: it authorizes contacts, opportunities, calendars, conversations, users read+write, and phone — the full surface; the agency PIT covers only agency-catalog scopes and is kept as a spare) on all GHL nodes — **verified live 2026-07-26**: the agency token authorizes the phone-system endpoints (your location key returned 401; either fix its scopes or just use the agency token, which the workflow now defaults to).
...
**State-pool gate (added 2026-07-26):** reps claim their state on the WeShare signup form — the selector is fed by live availability (`GET /api/public/state-pools`) and fully claimed states render disabled, so double-claiming is impossible at the source. Capacity defaults to 1 per state and is owner-tunable (AppSettings `state_pool_capacity` + per-state `state_pool_overrides` JSON) — raising it turns exclusivity into shared pools with zero code changes. The provisioner now opens with a hard gate: it verifies the partner's live claim against WeShare (`?verify=partnerCode`) and aborts before ANY purchase if the state is unclaimed or mismatched.
...
**First-run attempt log (2026-07-26):** executed live up to the wall that's yours to open — read the Demo User's real permissions (its `assignedDataOnly: True` matches the template; three deltas adopted from it: tags off, lead-value on, agent/reporting on, custom-menu links on; `affiliateManagerEnabled` deliberately NOT copied — confirm if you want it), fetched the agency companyId (now baked into the workflow), then the seat-create POST returned **401 "not authorized for this scope"** — the agency PIT can read users but lacks `users.write`. **Fix (30 seconds): Agency Settings → Private Integrations → edit the integration → add the Users write scope (and confirm Phone Numbers write while you're there) → save.** If GHL rotates the token when scopes change, update it in n8n + Coolify. Then re-fire the seat test. **UPDATE — seat test PASSED live (2026-07-26):** user `WeShare Test-Seat` created on the production location via the artifact's own payload; response verified 19/19 demo scopes, zero affiliate scopes, only-assigned-data on calendars/contacts/opportunities, role=user. Password policy discovered (upper+lower+digit+special) and baked into the generator. Remaining scope punch-list for the (new) Private Integration so the app + verify step work: **View Users, Contacts (view+edit), Opportunities (view+edit), Calendars (view+edit), Conversations/Messages (view+edit)** — phone, locations, and Edit Users confirmed working. Update `GHL_API_KEY` in Coolify and the n8n credential to the new token (the old integrations were wiped).
